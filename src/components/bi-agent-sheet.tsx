    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Modal Content */}
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-[90vw] sm:w-[85vw] lg:w-[1200px] xl:w-[1400px] max-w-[95vw] h-[90vh] flex flex-col overflow-hidden border border-slate-200 dark:border-slate-700">
            {/* Header */}
            <div className="border-b pb-6 flex-shrink-0 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-t-2xl px-6 pt-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="p-3 rounded-xl bg-primary/10 flex-shrink-0 shadow-sm">
                    {isLoading ? (
            ...
            <div className="flex-1 overflow-hidden">
              ...
              {!isLoading && !error && tabs.length > 0 && (
                <div className="py-8 px-2 space-y-8 h-full overflow-auto bg-slate-50/50 dark:bg-slate-900/50">
                  <Tabs
                    value={activeTab}
                  ...
                  >
                    <TabsList
                      className={`grid w-full ${tabsGridClass} flex-shrink-0 h-12 bg-white dark:bg-slate-800 shadow-sm border`}
                    >
                      {tabs.map((tab) => (
                        <TabsTrigger
                          key={tab.id}
                          value={tab.id}
                          className="text-sm font-medium px-6 py-3 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                        >
                    ...
                    <div className="flex-1 overflow-auto px-2 pt-6">
                      {tabs.map((tab) => (
                        <TabsContent
                          key={tab.id}
                          value={tab.id}
                          className="mt-0 space-y-8 h-full data-[state=active]:flex data-[state=active]:flex-col"
                        >
                          {/* Empty State */}
