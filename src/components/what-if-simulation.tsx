...
export function WhatIfSimulation({
  ...
  const [productCount, setProductCount] = useState([
    customer.featureValues.uniqueProducts,
  ]);
  const [avgSpend, setAvgSpend] = useState([
    Math.round(customer.featureValues.avgAmount),
  ]);
  const [frequency, setFrequency] = useState([
    customer.featureValues.transactionCount,
  ]);
  ...
  const runSimulation = () => {
    ...
    if (baselineAvg) {
      ...
      if (avgSpend[0] < Math.max(1, baselineAvg.mean * 0.5)) newScore -= 0.1;
    }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Adjust customer features to see impact on anomaly score
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pre-built scenarios and their predicted impact
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
